...
namespace DAL
{
    public class DALHopDongLaoDong
    {
        ...
        {
            var list = from hd in _databaseContext.HopDongLaoDongs
                       join nv in _databaseContext.NhanViens on hd.idNhanVien equals nv.id
                       select new
                       {
                           hd.id,
                           nv.TenNhanVien,
        ...
        public List<HopDongLaoDong> DsHopDongLaoDong() => _databaseContext.HopDongLaoDongs.ToList();

        public IQueryable<DTOHopDongLaoDong> DanhSachHopDongLaoDong() =>
        _databaseContext.HopDongLaoDongs.Select(p => new DTOHopDongLaoDong
        {
            Id = p.id,
        ...
        public bool Insert(DTOHopDongLaoDong dto)
        {
            try
            {
                ...
                _databaseContext.HopDongLaoDongs.InsertOnSubmit(entity);
        ...
        public bool Update(DTOHopDongLaoDong dto)
        {
            try
            {
                var entity = _databaseContext.HopDongLaoDongs.FirstOrDefault(x => x.id == dto.Id);
        ...
        public bool Delete(int id)
        {
            try
            {
                var entity = _databaseContext.HopDongLaoDongs.FirstOrDefault(x => x.id == id);
                if (entity == null) return false;
                _databaseContext.HopDongLaoDongs.DeleteOnSubmit(entity);
                _databaseContext.SubmitChanges();
                return true;
            }
            catch (Exception)
        ...
        {
            var q = from hd in _databaseContext.HopDongLaoDongs
                    join nv in _databaseContext.NhanViens on hd.idNhanVien equals nv.id
                    select new
                    {
                        hd.id,
